...
if (!function_exists('erased_language_catalog')) {
...
if (!function_exists('erased_language_select_options')) {
...
</style>'.($admin?'<link rel="stylesheet" href="/assets/admin-design-system.css?v=8.59">':'').($adminThemePkg?'<link rel="stylesheet" href="'.e($adminThemePkg['css_href']).'">':'').'
  ...
  (()=>{
   ...
   if(prefersNarrow){
    ...
    document.addEventListener("click", event => {
     if(!root.classList.contains("rail-hidden")){
      if(!event.target.closest(".rail") && !event.target.closest("[data-system-nav-toggle]")){
       root.classList.add("rail-hidden");
      }
     }
    });
   }
