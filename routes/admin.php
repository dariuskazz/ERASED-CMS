...
if($path==='/admin/languages'){require_permission('languages.manage');ensure_language_files();if($_SERVER['REQUEST_METHOD']==='POST'){verify_csrf();$action=isset($_POST['delete_language'])?'delete':($_POST['action']??'settings');if(isset($_POST['delete_language']))$_POST['delete_code']=$_POST['delete_language'];if($action==='generate_translation_base'){
 ...
 }if($action==='settings'){$catalog=erased_language_catalog();$adminCode=(string)($_POST['admin_language']??'en');$siteCode=(string)($_POST['site_language']??'en');if(!isset($catalog[$adminCode]))$adminCode='en';if(!isset($catalog[$siteCode]))$siteCode='en';$switcherVal=isset($_POST['show_language_switcher'])?'1':'0';$syncAdminVal=isset($_POST['admin_sync_site_language'])?'1':'0';set_setting('admin_language',$adminCode);set_setting('site_language',$siteCode);set_setting('show_language_switcher',$switcherVal);set_setting('nav_show_language',$switcherVal);set_setting('admin_sync_site_language',$syncAdminVal);set_setting('detect_browser_language',isset($_POST['detect_browser_language'])?'1':'0');
...
$h ='<div class="admin-stat-row">'
   ...
   .'<div class="panel"><div class="reg tl"></div><div class="reg tr"></div><div class="reg bl"></div><div class="reg br"></div>'
     ...
     .'<div class="panel-body">'
       ...
       .'<label class="check"><input type="checkbox" name="admin_sync_site_language" id="erasedAdminSyncCheckbox" value="1" '.(setting('admin_sync_site_language','1')==='1'?'checked':'').'> Apply selected website language to Admin Panel</label>'
